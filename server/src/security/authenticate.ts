...
export async function authenticateUsingAuthChain(
  ...
  if (chain.length === 0) {
    ...
    if (
      recoveredUserAddress.toString().toLowerCase() !==
      user.payload.toLowerCase()
    )
      return false

    // Recover address from entity signature
    const recoveredEphemeral = recover(
      signedEntity.payload,
    ...
    if (
      recoveredEphemeral.toString().toLowerCase() !==
...
function parseEmphemeralPayload(payload: string): {
  message: string
